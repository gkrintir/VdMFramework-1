...
class SimFixedCapSigma_PCCAndVtx_Fit(FitManager.FitProvider):
    ...
    def doFit( self,graph,config):
        ...
        #Remember: graph[0] is PCC, graph[1] is VTX

        ExpSigma = graph[1].GetRMS()*0.5
        ExpPeak = graph[1].GetHistogram().GetMaximum()
	...
	ff1.SetParNames("#Sigma","#sigma_{1}/#sigma_{2}","Amp","Frac","Mean")
        ...
        ff1.SetParameters(StartSigma,1.,StartPeak,StartFrac,0.)

        if LimitSigma_upper > LimitSigma_lower:
            ff1.SetParLimits(0, LimitSigma_lower,LimitSigma_upper)
        if LimitRatio_upper > LimitRatio_lower:
            ff1.SetParLimits(1, LimitRatio_lower,LimitRatio_upper)
        if LimitPeak_upper > LimitPeak_lower:
            ff1.SetParLimits(2, LimitPeak_lower,LimitPeak_upper)
        if LimitFrac_upper > LimitFrac_lower:
            ff1.SetParLimits(3, LimitFrac_lower,LimitFrac_upper)
...
#        r.gROOT.ProcessLine("gSystem->RedirectOutput(\".\/minuitlogtmp\/Minuit.log\", \"a\");")
        ...
        for j in range(5):
            fit = graph[1].Fit("ff1","S")
            if fit.CovMatrixStatus()==3 and fit.Chi2()/fit.Ndf() < 2: break

        r.gROOT.ProcessLine("gSystem->RedirectOutput(0);")
        
        sigma1 = ff1.GetParameter("#Sigma")
        m = ff1.GetParNumber("#Sigma")
        sigma1Err = ff1.GetParError(m)
        sigRatio1 = ff1.GetParameter("#sigma_{1}/#sigma_{2}")
        m = ff1.GetParNumber("#sigma_{1}/#sigma_{2}")
        sigRatio1Err = ff1.GetParError(m)
        amp1 = ff1.GetParameter("Amp")
        m = ff1.GetParNumber("Amp")
        amp1Err = ff1.GetParError(m)
        frac1 = ff1.GetParameter("Frac")
        m = ff1.GetParNumber("Frac")
        frac1Err = ff1.GetParError(m)
        mean1 = ff1.GetParameter("Mean")
        m = ff1.GetParNumber("Mean")
        mean1Err = ff1.GetParError(m)

        # Continue with fitting PCC within the VTX range 

        ExpSigma = graph[0].GetRMS()*0.5
        ExpPeak = graph[0].GetHistogram().GetMaximum()
	...
	ff.SetParNames("#Sigma","#sigma_{1}/#sigma_{2}","Amp","Frac","Mean","Const")
        ...
        ff.SetParameters(sigma1,sigRatio1,StartPeak,frac1,0.,StartConst)

        ff.SetParLimits(0, sigma1-sigma1Err,sigma1+sigma1Err)
        ff.SetParLimits(1, sigRatio1-sigRatio1Err,sigRatio1+sigRatio1Err)
        if LimitPeak_upper > LimitPeak_lower:
            ff.SetParLimits(2, LimitPeak_lower,LimitPeak_upper)
        ff.SetParLimits(3, frac1-frac1Err,frac1+frac1Err)
...
#        r.gROOT.ProcessLine("gSystem->RedirectOutput(\".\/minuitlogtmp\/Minuit.log\", \"a\");")
        ...
        xmax = r.TMath.MaxElement(graph[0].GetN(),graph[0].GetX())
