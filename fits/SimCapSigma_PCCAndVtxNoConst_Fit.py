...
class SimCapSigma_PCCAndVtxNoConst_Fit(FitManager.FitProvider):
...
# see http://root.cern.ch/phpBB3/viewtopic.php?f=14&t=14473, http://root.cern.ch/phpBB3/viewtopic.php?f=13&t=16844, https://agenda.infn.it/getFile.py/access?resId=1&materialId=slides&confId=4933 slide 23
        ...
        # [0]: #StartSigma[0], [1]: #sigma_{1}/#sigma_{2} =1, 2:StartPeak[0], 3: StartFrac[0], 4: Mean, 5: StartPeak[1], 6: Mean
        parffAndff1 = array.array('d', [StartSigma[0],1.,StartPeak[0],StartFrac[0],0., StartPeak[1], 0.])
        ...
        if LimitPeak_upper[1] > LimitPeak_lower[1]:
            fitter.Config().ParSettings(5).SetLimits(LimitPeak_lower[1],LimitPeak_upper[1]   )
        ...
        # 0: #Sigma, 1: #sigma_{1}/#sigma_{2}, 2: Amp, 3: Frac, 4: Mean
        parff_draw = array.array('i', [0,1,2,3,4])
        fittedFunctions[0].SetFitResult( result, parff_draw);
        # 0: #Sigma (common parameter), 1: #sigma_{1}/#sigma_{2} (common parameter), 5: Amp, 3: Frac (common parameter), 6: Mean
        parff1_draw = array.array('i', [0,1,5,3,6])
