...
class  SimRatio_PCCAndVtx_Fit(FitManager.FitProvider):
    ...
    def doFit( self,graph,config):
        ...
        nPars1 = 6
        nPars2 = 5
        nCommonPars = 1 #(sigma_{1}/sigma_{2})
        ...
        for i in range(0,2):
            ...
            StartConst.append( config['StartConst'] )                                                                                                                                          
            LimitConst_lower.append( config['LimitsConst'][0] )                                                                                                                                
            LimitConst_upper.append( config['LimitsConst'][1] )  
...
# see http://root.cern.ch/phpBB3/viewtopic.php?f=14&t=14473, http://root.cern.ch/phpBB3/viewtopic.php?f=13&t=16844, https://agenda.infn.it/getFile.py/access?resId=1&materialId=slides&confId=4933 slide 23
        ...
        # [0]: #StartSigma[0], [1]: #sigma_{1}/#sigma_{2} =1, 2:StartPeak[0], 3: StartFrac[0], 4: Mean, 5: StartConst[0], 6: StartSigma[1], 7: StartPeak[1], 8: StartFrac[1], 9: Mean
        parffAndff1 = array.array('d', [StartSigma[0],1.,StartPeak[0],StartFrac[0],0., StartConst[0], StartSigma[1], StartPeak[1], StartFrac[1],0.])
        ...
        if LimitConst_upper[0] > LimitConst_lower[0]:                                                                                                                                          
            fitter.Config().ParSettings(5).SetLimits(LimitConst_lower[0],LimitConst_upper[0])   
        ...
        # 6: #Sigma, 1: #sigma_{1}/#sigma_{2} (common parameter), 7: Amp, 8: Frac, 9: Mean, 
        parff1_draw = array.array('i', [6,1,7,8,9])
        ...
        for i in range(0,2):
            ...
            if (i==0):
                const = fittedFunctions[i].GetParameter("Const")                                                                                                                               
		m = fittedFunctions[i].GetParNumber("Const")                                                                                                                                   
                constErr = fittedFunctions[i].GetParError(m)                                                                                                                                   
            else:
                const = -999.
                constErr = -999.
